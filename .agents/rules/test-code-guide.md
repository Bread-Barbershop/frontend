...
- 커스텀 훅은 `@testing-library/react의` renderHook을 사용한다.
...
| # | 시나리오 | 검증 포인트 |
|---|----------|-------------|
| 1 | Google 로그인 | OAuth 인증 → 로그인 성공 → 메인 화면 진입 |
| 2 | 초대장 제작 | 템플릿 선택 → 내용 입력 → 미리보기 확인 |
| 3 | 업로드 | Google Drive 업로드 → 성공 응답 확인 |
| 4 | URL 발행 | 공유 URL 생성 → 클립보드 복사 → URL 접근 시 초대장 렌더링 |
| 5 | 전체 플로우 | 로그인 → 제작 → 업로드 → URL 발행 E2E 통과 | 4. 공통 규칙
테스트 작성 원칙
